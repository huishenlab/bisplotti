...
plotEpiread <- function(mat, plot_read_ave = TRUE,
  ...
  if (!is(mat, "matrix")) {
    ...
    # FIXME: When show_filtered = FALSE, then the CG and GC plots can be offset from one another (not have the same
    #        number of lines), need to find a way to draw just the line when there are points in one table but not the
    #        other. Show filtered should only turn on/off the dots themselves
    # Ideally, show_filtered should match the value of filter_empty_reads in tabulateEpibed
...
.set_ql_theme <- function(show_readnames = T, show_positions = T) {
...
.epiClustPlot <- function(matplotdata,
  ...
  plt <- ggplot(matplotdata, aes(x = Var2, y = Var1)) +
      ...
      guides(color = "legend") +
      geom_label(
          data = subset(matplotdata, value %in% snp_list),
          aes(label = value)
      ) +
      ql_theme
  ...
  if (any(matplotdata$value %in% c("M", "U"))) {
      plt <- plt +
          scale_fill_manual(
              values = c(M = meth_color, U = unmeth_color), na.value = na_color
          )
  } else {
      plt <- plt +
          scale_fill_manual(
              values = c(O = meth_color, S = unmeth_color), na.value = na_color
          )
  }
  return(plt)
}
